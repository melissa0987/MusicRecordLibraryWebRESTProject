...
public class RecordResources {
	...
	//saves records data from input file
	private void saveRecordList() throws IOException {
	    // Get the real path to the data file
	    String filePath = context.getRealPath("/WEB-INF/data/records.txt");

	    try (PrintWriter writer = new PrintWriter(new FileWriter(filePath, false))) {
	        for (Record record : recordList) {
	            // Write each record in the same tab-separated format
	...
	//standard html table for record table
  ...
  //standard html wrapper: head, body and some script
    ...
    @Path("/allRecordsJSON")
    @Produces(MediaType.APPLICATION_JSON)
    public List<Record> getAllRecordsJSON() throws FileNotFoundException {
    	loadRecordList();
        return recordList;
    }
...
//  REST 
    ...
    public String addNewRecordInfo(
        ...
        @FormParam("totalCopies") int totalCopies) throws IOException {
    ...
    public String updateRecordInfo(
        ...
        @QueryParam("totalCopies") int totalCopies) throws IOException {
    ...
    @Produces(MediaType.TEXT_HTML)
    public String deleteRecord(@QueryParam("recordID") String recordID) throws IOException {
        loadRecordList();
        String message = "<p style='color:red;'>No record found with ID: " + recordID + "</p>";
